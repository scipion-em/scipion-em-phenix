...
"""
    ...
    """
        Refinement Base (PhenixProtRunRefinementBase) — User Manual

            Overview

            The PhenixProtRunRefinementBase protocol provides the common
            infrastructure used by Phenix refinement and validation workflows
            within Scipion. Its main purpose is to prepare cryo-EM maps and
            atomic structures, execute MolProbity and validation routines,
            and collect structural quality statistics in a standardized way.

            In practical cryo-EM workflows, this protocol acts as the base
            layer for refinement procedures that evaluate how well an atomic
            model fits the experimental density while preserving correct
            stereochemical geometry. The protocol focuses on validation
            metrics that are biologically meaningful and widely used in
            structural biology publications.

            Inputs and General Workflow

            The protocol requires an input atomic structure in PDB or mmCIF
            format together with an optional cryo-EM density map. If no map
            is explicitly provided, the protocol automatically retrieves the
            volume associated with the input structure. The user also defines
            the experimental resolution, which is required for validation
            and refinement procedures.

            During execution, the protocol converts the input map into MRC
            format while preserving voxel sampling and spatial origin
            information. This normalization step guarantees compatibility
            with downstream Phenix programs and avoids inconsistencies caused
            by map formatting differences.

            Validation and Geometry Analysis

            A major objective of the protocol is the extraction of
            stereochemical validation metrics using MolProbity and Phenix
            validation tools. The protocol evaluates Ramachandran statistics,
            rotamer quality, C-beta deviations, clashscore, and the overall
            MolProbity score. These metrics are commonly used to estimate
            the reliability of atomic models derived from cryo-EM maps.

            From a biological perspective, low numbers of Ramachandran and
            rotamer outliers indicate realistic backbone and side-chain
            conformations, while low clashscores suggest that the model does
            not contain severe steric overlaps. The overall MolProbity score
            provides a global estimation of model quality and geometric
            consistency.

            Validation File Processing

            The protocol supports both direct MolProbity output parsing and
            Cryo-EM validation pickle files generated by Phenix. When pickle
            validation files are available, the protocol automatically
            extracts the relevant statistics and converts them into Scipion
            objects that can later be displayed or reused by downstream
            protocols.

            This design allows compatibility across different Phenix
            versions while maintaining a unified interface for users working
            in heterogeneous cryo-EM processing environments.

            Parallel Execution and External Programs

            The protocol supports multithreaded execution through the nproc
            parameter, allowing faster validation on large atomic models.
            It also verifies that the required Phenix binaries are correctly
            installed and accessible from the Scipion configuration.

            If the required Phenix executables are missing or incompatible,
            the protocol reports detailed configuration errors to simplify
            troubleshooting and environment validation.

            Outputs and Their Interpretation

            After execution, the protocol stores the extracted validation
            statistics and generates a summary describing the stereochemical
            quality of the refined model. These results can be reused by
            refinement protocols derived from this base class or inspected
            directly by the user.

            Biologically, these outputs help determine whether a refined
            structure is suitable for interpretation, comparative analysis,
            or deposition into structural databases.

            Final Perspective

            The PhenixProtRunRefinementBase protocol is not intended to
            perform refinement directly, but instead provides the common
            validation and preprocessing framework required by multiple
            Phenix-based refinement workflows in Scipion. By standardizing
            map preparation, geometry validation, and result extraction,
            the protocol ensures reliable quality assessment of cryo-EM
            atomic structures across different refinement strategies.
        """
